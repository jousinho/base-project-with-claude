# sf8/postgresql-messenger-redis-example — Symfony 8 + PostgreSQL + Messenger (Redis transport) + DDD completo
...
Para partir de cero sin código de dominio, usa `sf8/postgresql-messenger-redis`.
Para la versión con Symfony 7, usa `sf7/postgresql-messenger-redis-example`.
...
```bash
# 1. Clona la rama
git clone -b sf8/postgresql-messenger-redis-example git@github.com:jousinho/base-project-with-claude.git mi-proyecto
cd mi-proyecto
