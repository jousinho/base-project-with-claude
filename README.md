# sf7/mysql-example — Symfony 7 + MySQL + DDD completo
...
Para partir de cero sin código de dominio, usa `sf7/mysql`.
Para la versión con Symfony 8, usa `sf8/mysql-example`.
...
```bash
# 1. Clona la rama
git clone -b sf7/mysql-example git@github.com:jousinho/base-project-with-claude.git mi-proyecto
cd mi-proyecto
...
**Entidades no son `final` en ramas sf7.** PHP 8.3 no soporta lazy ghost objects nativos. Doctrine necesita extender la clase para generar proxies de lazy loading, lo que es incompatible con `final`. PHP 8.4 resuelve esto; en las ramas `sf8` las entidades sí son `final`.
